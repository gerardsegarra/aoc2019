...
def get_crosses(cable1, cable2):
    return list(set(position1) & set(position2))

def next_current_position(x, y, direction):
    if direction == "R":
        return x + 1, y
...
def run_cable(cable):
    ...
    positions = []
    for movement in cable:
        direction = movement[0]
        turns = int(movement[1:])
        current_trun = 0
        while current_trun < turns:
            current_position = next_current_position(current_position[0], current_position[1], direction)
            positions.append(current_position)
            current_trun = current_trun + 1
    return positions
position1 = run_cable(cable1)
position2 = run_cable(cable2)


crosses = get_crosses(position1, position2)
dist = []

for cross in crosses:
    dist.append(position1.index(cross) + position2.index(cross) + 2)
